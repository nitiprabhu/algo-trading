"""
DigitalOcean Serverless Function: Power On Droplet
Scheduled trigger: Mon-Fri at 03:00 UTC (08:30 AM IST)
"""
...
def main(args):
    ...
    if not token:
        msg = "🚨 *[DO Functions] Missing `DO_API_TOKEN`.* Cannot power on droplet."
    ...
    payload = json.dumps({"type": "power_on"}).encode("utf-8")
    req = urllib.request.Request(
        url,
        data=payload,
        headers={
    ...
    try:
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            msg = (
                f"🟢 *[DO Functions] Droplet Power ON Triggered*\n\n"
                f"• *Droplet ID:* `{droplet_id}`\n"
                f"• *Action Status:* `{action_status}`\n"
                f"• *Schedule:* Market Pre-Open Ready (08:30 AM IST)\n"
        ...
        if "already" in err_msg.lower() and "on" in err_msg.lower():
            msg = (
                f"ℹ️ *[DO Functions] Droplet Already Active*\n\n"
                f"• *Droplet ID:* `{droplet_id}`\n"
                f"• *Status:* Droplet is already powered ON."
            )
            send_telegram(tg_token, tg_chat_id, msg)
            return {"statusCode": 200, "body": {"message": "Droplet already powered on"}}

        msg = (
            f"🚨 *[DO Functions] Power ON Failed!*\n\n"
            f"• *Droplet ID:* `{droplet_id}`\n"
            f"• *HTTP Status:* `{e.code}`\n"
            f"• *Error:* `{err_msg}`\n\n"
            f"⚠️ Trading day at risk! Check DigitalOcean console immediately."
        )
        send_telegram(tg_token, tg_chat_id, msg)
        ...
        msg = (
            f"🚨 *[DO Functions] Unexpected Error during Power ON*\n\n"
            f"• *Droplet ID:* `{droplet_id}`\n"
            f"• *Error:* `{str(e)}`"
        )
        send_telegram(tg_token, tg_chat_id, msg)
