...
WEEKS = [
    ("Jun 15 - Jun 19", datetime(2026, 6, 15, tzinfo=IST), datetime(2026, 6, 19, tzinfo=IST)),
    ("Jun 22 - Jun 26", datetime(2026, 6, 22, tzinfo=IST), datetime(2026, 6, 26, tzinfo=IST)),
    ("Jun 29 - Jul 03", datetime(2026, 6, 29, tzinfo=IST), datetime(2026, 7, 3, tzinfo=IST)),
]
...
async def main():
    ...
    print("\n" + "=" * 70)
    print("📊 3-WEEK PERFORMANCE COMPARISON (post-fix code) — Jun 15 to Jul 03, 2026")
    print("=" * 70)
    ...
    for label, start_date, end_date in WEEKS:
        ...
        print("-" * 70)
        print(f"{'WEEK TOTAL':12s} | ₹{week_opt:<11.2f} | ₹{week_fut:<11.2f} | ₹{(week_opt+week_fut):<11.2f} | {week_trades}")

        grand_opt += week_opt
        grand_fut += week_fut
        grand_trades += week_trades

    print("\n" + "=" * 70)
    print(f"{'GRAND TOTAL':12s} | ₹{grand_opt:<11.2f} | ₹{grand_fut:<11.2f} | ₹{(grand_opt+grand_fut):<11.2f} | {grand_trades}")
    print("=" * 70 + "\n")
