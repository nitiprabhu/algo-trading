...
async def main():
    # 1. Run April Backtest (April 1 - April 30)
    april_start = datetime(2026, 4, 1, tzinfo=IST)
    april_end = datetime(2026, 4, 30, 15, 30, tzinfo=IST)
    april_res = await run_backtest_for_range(april_start, april_end)
    ...
    # 3. Run June Backtest (June 1 - June 15)
    june_start = datetime(2026, 6, 1, tzinfo=IST)
    june_end = datetime(2026, 6, 15, 15, 30, tzinfo=IST)
    ...
    msg = (
        ...
        "*📅 April 2026 Range (01 Apr - 30 Apr):*\n"
        f"  - Options PnL: `₹{april_res.get('options', 0.0):+,.2f}`\n"
        f"  - Futures PnL: `₹{april_res.get('futures', 0.0):+,.2f}`\n"
        f"  - Combined PnL: `₹{april_res.get('combined', 0.0):+,.2f}`\n"
        f"  - Total Trades: `{april_res.get('trades', 0)}` | Win Rate: `{april_res.get('win_pct', 0.0)}%`\n\n"
