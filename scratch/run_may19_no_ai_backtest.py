...
async def main():
    ...
    # Set thresholds to 0.55
    for key in list(runtime.signal_engine.thresholds.keys()):
        runtime.signal_engine.thresholds[key] = 0.55
    runtime.signal_engine.thresholds["DEFAULT"] = 0.55
    ...
    print(f"\n{'='*70}")
    print(f"🚀 INITIALIZING {DATE_STR} BACKTEST (NO AI, THRESHOLD: 0.55)")
    print(f"{'='*70}")
    ...
    print(f"\n{'='*70}")
    print(f"📊 PERFORMANCE METRICS FOR {DATE_STR} (NO AI, THRESHOLD: 0.55)")
    print(f"{'='*70}")
